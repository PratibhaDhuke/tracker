...
import NotesList from "./NotesList"; // Import the NotesList component
...
const Home = () => {
  ...
  const [showNotes, setShowNotes] = useState(false); // Control visibility of Notes List
  ...
  return (
    <div className="Home">
          ...
          {habits.map((habit) => (
            <tr key={habit.name}>
              ...
              <td>
                ...
                {
                  Object.keys(habit.completed).filter(
                    (date) =>
                      habit.completed[date] &&
                      calendarDays.some(
      ...
      {/* Button to toggle the visibility of the Notes List */}
      <button
      ...
      {/* Conditionally render the Notes Display */}
